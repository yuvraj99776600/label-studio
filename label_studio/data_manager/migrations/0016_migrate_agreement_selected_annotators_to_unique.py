...
def forward_migration():
    migration, created = AsyncMigrationStatus.objects.get_or_create(
        name=migration_name, defaults={"status": AsyncMigrationStatus.STATUS_STARTED}
    )
    if not created:
    ...
    for row in qs:
        ...
        if project_id not in project_to_unique_annotators:
            unique_ids = set(
                Annotation.objects.filter(project_id=project_id, completed_by_id__isnull=False)
...
class Migration(migrations.Migration):
    ...
    dependencies = [("data_manager", "0015_alter_view_options")]
