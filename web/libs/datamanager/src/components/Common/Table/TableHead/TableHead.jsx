...
const ColumnRenderer = observer(
  ({
    ...
    const extra =
      !isDE && columnHeaderExtra ? columnHeaderExtra(column, Decoration) : null;
    ...
    const headContent = (
      <>
        ...
        {extra && (
          <span className={tableHeadCN.elem("column-extra").toString()}>
            {extra}
          </span>
        )}
      </>
    ...
    const isAgreementSelected =
      isActive(FF_AGREEMENT_FILTERED) && column.type === "AgreementSelected";
      ...
      <TableCell data-id={id} mix="th">
        ...
        >
          {!isDE && column.parent ? (
            ...
            <AgreementSelectedWrapper column={column}>
              {headContent}
            </AgreementSelectedWrapper>
          ) : (
            headContent
          )}
        </Resizer>
...
export const TableHead = observer(
  forwardRef(
    (
      ...
      const { columns, headerRenderers, cellViews } =
            ...
            return (
              <span
                ...
                onDragEnd={(e) => {
                  e.stopPropagation();
                  const draggedCol = states.getDraggedCol();
                  const curColumns = columns.filter(
                    (curCol) => curCol.id !== draggedCol,
                  );
                  const newIndex = curColumns.findIndex((curCol) => {
                    const colRefrence = colRefs.current[curCol.id];
                    const mousePos =
                      e.clientX +
                      (ref?.current?.parentElement?.parentElement.scrollLeft ??
                        0);
                    const isGreaterThanPos =
                      mousePos <
                      colRefrence.offsetLeft + colRefrence.clientWidth / 2;
