# MLTL Annotate Frontend

MLTL Annotate Frontend (LSF) is a crucial module of the MLTL Annotate ecosystem, pivotal in driving the entire annotation flow. It's a front-end-only module, combining a user interface for annotation creation with a data layer that standardizes the annotation format. Every manual annotation in MLTL Annotate has been crafted using LSF, making it integral to the system.
...
- **`yarn lsf:watch`: Build LSF continuously**
  - Crucial for development, this script continuously builds MLTL Annotate Frontend (LSF), allowing developers to observe their changes in real-time within the MLTL Annotate environment.
- **`yarn lsf:serve`: Run LSF standalone**
  - To run MLTL Annotate Frontend in standalone mode. Visit http://localhost:3000 to use the application in standalone mode.
- **`yarn lsf:e2e`: Execute end-to-end (e2e) tests on LSF**
  - To run comprehensive e2e tests, ensuring the frontend works as expected from start to finish. The MLTL Annotate environment must be running, typically at `http://localhost:8080`.
