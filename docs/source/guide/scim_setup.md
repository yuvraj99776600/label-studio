---
title: Set up SCIM2 for MLTL Annotate
short: SCIM2
tier: enterprise
type: guide
order: 0
order_enterprise: 389
meta_title: System for Cross-domain Identity Management (SCIM) for MLTL Annotate
meta_description: MLTL Annotate documentation for setting up SCIM2
section: "Manage Your Organization"
parent: "admin_auth"
parent_enterprise: "admin_auth"
---

System for Cross-domain Identity Management (SCIM) is a popular protocol to manage access for services and applications across an organization. This guide helps to set up SCIM integration to manage access to MLTL Annotate in your organization. 

Using SCIM provider, you can manage access to MLTL Annotate workspaces, grant roles to individual users and groups.
...
MLTL Annotate uses SCIM Version 2.0 standard. As an example, this page uses integration with [Okta](https://www.okta.com/integrate/). However, MLTL Annotate follows [SCIM RFC 5741](https://datatracker.ietf.org/doc/html/rfc7644#section-3.2) and can be integrated with any access management services that support the standard.

For more information on how SCIM workflows, see [How SCIM works with MLTL Annotate](scim_workflow). 
...
To manage access to MLTL Annotate, add the application to your SCIM provider (Okta). Okta uses Bearer (request header should be `Authorization: Bearer <token>`) token to interact with REST API endpoints of the application to provision and deprovision access.

### Add MLTL Annotate (if not complete)

1. Navigate to **Applications → Applications** in Okta. Click  **Create App Integration**. 
2. Select **SAML 2.0**. Fill App name (for example, _MLTL Annotate_).
3. On the next step **Configure SAML** set up SAML integration following the instructions to [Set up SSO guide](auth_setup.html).
4. Make sure MLTL Annotate appears in the list of active applications.

### Enable SCIM provisioning

1. Navigate to **Applications → Applications** in Okta. 
2. Select **MLTL Annotate**.
...
Fill in the fields:
 - **SCIM connector base URL**: `https://<LABEL_STUDIO_BASE_URL>/scim/v2/` where `<LABEL_STUDIO_BASE_URL>` is the base URL of your MLTL Annotate instance.
 - **Unique identifier field for users**: Use `email`. MLTL Annotate uses email as user identifier in this field.
 - **Supported provisioning actions**: Select the following items:
   - Import New Users and Profile Updates
   - Push New Users
   - Push Profile Updates
   - Push Groups
 - **HTTP Header → `Authorization: Bearer <token>`**: Put the Bearer token from the LSE owner's account profile. In case of MLTL Annotate `Token` and `Bearer` are the same tokens. However, it's important to use `Bearer` instead of `Token` in the request header. 
...
3. Select the people you would like to be added to MLTL Annotate.
4. Click **Done**.

After you click **Done**, Okta will send the requests to create users accordingly in the MLTL Annotate.
...
The most convenient way to manage access to the application is via groups. You can assign MLTL Annotate to groups and manage the groups in Okta. The changes will be propagated to the application.

### Set up group mapping

1. In MLTL Annotate, click the menu in the upper left and select **Organization**. 

    ![Screenshot of Organization in the MLTL Annotate menu](/images/general/menu_organization.png)
...
3. Update roles and workspaces mapping. Ensure the group name you enter is the same as the group name being sent by your SCIM provider. 
    ...
    * **Organization Roles to Groups Mapping**---Map groups to roles at the organization level. The role set at the organization level is the default role of the user and is automatically assigned to workspaces and projects. For more information on roles, see [Roles in MLTL Annotate](admin_roles). 
    ...
    * **Projects to Groups Mapping**---Map groups to roles at the project level. Project-level roles can be **Annotator**, **Reviewer**, or **Inherit**. 
        ...
        You can map a group to different roles across multiple projects. You can also map multiple groups to the same roles and the same projects. For more information on roles, see [Roles in MLTL Annotate](admin_roles). 
...
After saving the group assignment, the update will be queued and sent to MLTL Annotate. 

!!! note
    Alternatively, you can push the changes immediately to MLTL Annotate.
...
3. Find the group you would like to sync to MLTL Annotate. 
