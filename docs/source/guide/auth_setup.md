---
title: Set up SSO authentication for MLTL Annotate
short: SSO and SAML
tier: enterprise
type: guide
order: 0
order_enterprise: 387
meta_title: SSO authentication for MLTL Annotate
meta_description: MLTL Annotate documentation for setting up SSO authentication for your data labeling, machine learning, and data science projects.
section: "Manage Your Organization"
parent: "admin_auth"
parent_enterprise: "admin_auth"
---

Set up single sign-on using SAML to manage access to MLTL Annotate using your existing Identity Provider (IdP).

!!! error Enterprise
    SSO authentication is only available in MLTL Annotate Edition. If you're using Label Studio Community Edition, see <a href="https://docs.mltl.us/guide/label_studio_compare.html">Label Studio Features</a> to learn more.


To more easily [manage access to MLTL Annotate](manage_users.html), you can map SAML groups to both `roles` or `workspaces`. 

## Set up SAML SSO

The organization Owner or Administrator for MLTL Annotate can set up SSO & SAML for the instance. MLTL Annotate supports the following IdPs:
...
After setting up the SSO, you can use native authentication to access the MLTL Annotate UI, however it's not a recommended option especially for the user with the Owner role.
...
### Connect your Identity Provider to MLTL Annotate

Set up MLTL Annotate as a Service Provider (SP) with your Identity Provider (IdP) to use SAML authentication. 
...
4. Copy the following URLs:
    ...
    * **Login URL**---This is the URL that users will use to log in to MLTL Annotate. 
    * **Logout URL**---This is the URL used to redirect users after successfully logging out of MLTL Annotate.

###### From your IdP:

1. Paste the URLs copied from MLTL Annotate in the appropriate location. 
2. Generate a metadata XML file, or a URL that specifies the metadata for the IdP.
3. Set up or confirm setup of the following SAML attributes. MLTL Annotate expects specific attribute mappings for user identities.
...
!!! note Note
    Different Identity Providers use different attribute names. MLTL Annotate provides **presets** in the SSO & SAML settings page to quickly configure the correct attribute mappings for popular IdPs. You can also manually configure custom attribute names if your IdP uses different values.
...
3. Set up group mappings. These can also be added or edited later.
    ...
    * **Projects to Groups Mapping**---Map groups to roles at the project level. Project-level roles can be **Annotator**, **Reviewer**, or **Inherit**. 
        ...
        You can map a group to different roles across multiple projects. You can also map multiple groups to the same roles and the same projects. For more information on roles, see [Roles in MLTL Annotate](manage_users#Roles-in-Label-Studio-Enterprise). 
...
5. Test the configuration by logging in to MLTL Annotate with your SSO account.
...
If you want to manage MLTL Annotate roles and workspaces entirely with single sign-on (SSO), add the following to your environment variable file:
...
!!! info Tip
    If you are using the SaaS version of MLTL Annotate (MLTL Annotate Cloud) and would like to enable these restrictions for your organization, [open a ticket](https://mltl.us/support/hc/en-us/requests/new) to submit your request.  
