...
meta_title: MLTL Annotate Webhook Event Reference 
meta_description: MLTL Annotate reference documentation for webhook event fields and payloads sent from MLTL Annotate for integration with your machine learning pipeline. 
...
MLTL Annotate includes several types of webhook events that trigger when specific actions occur. Refer to the details on this page to determine what information is included in the payload of each webhook event. For details about how to use webhooks and to set up webhooks in MLTL Annotate, see [Set up webhooks in MLTL Annotate](webhooks.html). To create your own webhooks in MLTL Annotate to trigger events when specific actions happen, see [Create custom events for webhooks in MLTL Annotate](webhook_create.html). 

Webhooks sent from MLTL Annotate include the following:
...
The HTTP POST payloads that MLTL Annotate sends to the configured webhook URLs include the headers that you set up when you [configure the webhook](webhooks.html).
...
Sent when a task is created in MLTL Annotate. See how to [set up a webhook for this event](webhooks.html). 
...
Sent when a task is deleted from MLTL Annotate. See how to [set up a webhook for this event](webhooks.html).
...
## Annotation Created
Sent when an annotation is created for a task in MLTL Annotate. See how to [set up a webhook for this event](webhooks.html).
