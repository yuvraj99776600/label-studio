...
MLTL Annotate uses SQLite by default. You don't need to configure anything. MLTL Annotate stores all data in a single file in the specified directory of the admin user. After you [start MLTL Annotate](start.html), the directory used is printed in the terminal. 
...
Run the following command to launch MLTL Annotate, configure the connection to your PostgreSQL database, scan for existing tasks, and load them into the app for labeling for a specific project.

```bash
label-studio start my_project --init -db postgresql 
```

You must set the following environment variables to connect MLTL Annotate to PostgreSQL:
...
When you start MLTL Annotate using Docker Compose, you start it using a PostgreSQL database:
```bash
docker-compose up -d
```
...
An example docker-compose file for this is available in the [MLTL Annotate repository](https://github.com/yuvraj99776600/label-studio).

To run MinIO alongside your MLTL Annotate instance, use the following command:
````bash
...
### Connect MLTL Annotate to local MinIO

If you do not have a static IP address, create an entry in your hosts file so that both the MLTL Annotate container and 
...
If you're using a Docker container, Heroku, or another cloud provider, you might want your data to persist after shutting down MLTL Annotate. You can [export your data](export.html) to persist your labeling task data and annotations, but to preserve the state of MLTL Annotate and assets such as files that you uploaded for labeling, set up data persistence. 

### Persist data with Docker

Mount Docker volumes on your machine to persist the internal SQLite database and assets that you upload to MLTL Annotate after you terminate a Docker container running MLTL Annotate. 
...
If you're using Docker Compose with the [config included in the MLTL Annotate repository](https://github.com/yuvraj99776600/label-studio/blob/develop/docker-compose.yml), you can set up Docker volumes in the `docker-compose.yml` file for MLTL Annotate:
```
version: "3.3"
services:
  label_studio:
...
Host a PostgreSQL server that you manage and set up the PostgreSQL environment variables with MLTL Annotate to persist data from a cloud provider such as Heroku, Amazon Web Services, Google Cloud Services, or Microsoft Azure. 
