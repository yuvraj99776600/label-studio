...
meta_description: Access tokens to interact with the MLTL Annotate API and SDK. 
section: "Manage Your Organization"
date: 2025-02-18 12:03:59
parent_enterprise: "admin_settings"
---

MLTL Annotate has personal access tokens and legacy tokens. These tokens are also referred to as your "API keys." 
...
<thead>
  ...
  <td>
    <ul>
        <li>Have a TTL that can be set at the org level. (MLTL Annotate only)
...
* When a certain token type is disabled, existing tokens will not be able to authenticate to the MLTL Annotate platform. 
...
```python
# Define the URL where MLTL Annotate is accessible and the API key for your user account
...
# Connect to the MLTL Annotate API 
...
```http
curl -X <method> <MLTL Annotate URL>/api/<endpoint> -H 'Authorization: Bearer your-new-access-token'
```
