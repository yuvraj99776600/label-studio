...
meta_title: Using SAM2 with MLTL Annotate for Video Annotation
categories:
    - Computer Vision
    - Video Annotation
    - Object Detection
...
# Using SAM2 with MLTL Annotate for Video Annotation

This guide describes the simplest way to start using **SegmentAnything 2** with MLTL Annotate.

This repository is specifically for working with object tracking in videos. For working with images, 
see the [segment_anything_2_image repository](https://github.com/yuvraj99776600/label-studio-ml-backend/tree/master/label_studio_ml/examples/segment_anything_2_image)
...
This tutorial uses the [`segment_anything_2_video` example](https://github.com/yuvraj99776600/label-studio-ml-backend/tree/master/label_studio_ml/examples/segment_anything_2_video). 
...
- LABEL_STUDIO_URL: the http:// or https:// link to your MLTL Annotate instance (include the prefix!) 
- LABEL_STUDIO_API_KEY: your api key for MLTL Annotate, available in your profile. 
...
5. Connect running ML backend server to MLTL Annotate: go to your project `Settings -> Machine Learning -> Add Model` and specify `http://localhost:9090` as a URL. Read more in the official [MLTL Annotate documentation](https://docs.mltl.us/guide/ml#Connect-the-model-to-Label-Studio).
