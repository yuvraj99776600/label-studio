...
| --- | --- | --- |
| **Total Submitted Annotations** | Sum of `submitted_or_reviewed` | The total number of annotations the user submitted during the selected time frame. <br /><br />This includes annotations that have been submitted or updated, either reviewed or not. It does not include annotations that have been skipped. <br /><br />Note that each annotation is only included in their submitted count once. MLTL Annotate does not count the same annotation twice based if it is later updated. | 
