...
When a user interacts with the model, you can import the user prompt and the model response into MLTL Annotate and then use a labeling interface designed for a response moderation task.
...
To create evaluation task from LLM response and import it into the created MLTL Annotate project, you can use the format in the following example:
...
# the task to import into MLTL Annotate
task = {'chat': messages}
```
