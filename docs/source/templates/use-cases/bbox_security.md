...
cat: computer-vision
order: 1103
meta_description: Template for using MLTL Annotate to perform object detection with rectangular bounding boxes for security systems.
---
...
However, the data labeling process for object detection is often fraught with challenges, including time-intensive annotation tasks that can lead to inconsistencies, as well as the need for domain expertise to accurately identify and classify objects within the visual context. MLTL Annotate effectively addresses these issues with its innovative hybrid AI-assisted pre-labeling feature, which significantly accelerates the annotation process by generating initial bounding box suggestions. Furthermore, our platform empowers expert reviewers to validate and refine these labels, ensuring high-quality, consistent results. With robust collaboration tools and customizable labeling templates tailored to the security domain, MLTL Annotate not only streamlines workflows but also enhances expert efficiency and scalability, ultimately improving model performance while reducing the time required for data preparation.
...
This configuration is wrapped inside <a href="https://docs.mltl.us/tags/view">View</a> tags.

Use the <a href="https://docs.mltl.us/tags/image">Image</a> element to specify the surveillance footage or camera snapshot to annotate:

```xml
<Image name="image" value="$image"/>
```

Use the <a href="https://docs.mltl.us/tags/rectanglelabels">RectangleLabels</a> tag to define labels and attach rectangular bounding boxes to the security image data. Use <a href="https://docs.mltl.us/tags/label">Label</a> tags to control the color coding for different security categories:
...
Once an analyst selects a box, the <a href="https://docs.mltl.us/tags/header">Header</a> provides instructions for describing the detected item.

The <a href="https://docs.mltl.us/tags/textarea">TextArea</a> control allows entry of textual notes per selected region, specified via <code>perRegion="true"</code>. You may also add a <code>placeholder</code> attribute for suggested text prompts.

Additionally, use the <a href="https://docs.mltl.us/tags/choices">Choices</a> tag with <code>perRegion</code> enabled to capture status assessments related to the bounding box content.
