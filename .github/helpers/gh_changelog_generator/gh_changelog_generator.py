...
def render_output_md(
    ...
    for label, tasks in sorted(
        sorted_release_tasks.items(), key=lambda x: LABEL_SORT.index(x[0]) if x[0] in LABEL_SORT else 100
    ):
        release_notes_lines.extend(render_add_header_md(label, render_tasks_md(tasks)))
    ...
    if missing_in_gh:
        comment.extend(
            render_add_spoiler_md(f"Missing in GitHub release ({len(missing_in_gh)})", render_tasks_md(missing_in_gh))
        )
    if missing_in_tracker:
        comment.extend(
            render_add_spoiler_md(
                f"Missing in Task Tracker release ({len(missing_in_tracker)})", render_tasks_md(missing_in_tracker)
            )
        )
    ...
    if turned_off_feature_flags:
        comment.extend(
            render_add_spoiler_md(
                f"Turned off Feature Flags ({len(turned_off_feature_flags)})", render_ffs_md(turned_off_feature_flags)
            )
        )
    ...
    release_notes_lines.extend(render_add_quote_md(comment))
...
def render_output_json(
    ...
    result = {"sorted_release_tasks": sorted_release_tasks_json}
    return result
